...
export default function UserListChat({ onSelect, selectedId, unreadCounts = {} }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      const botUser: UserProfile = {
        ...
        email: null,
        avatar_url: "https://api.dicebear.com/7.x/adventurer/svg?seed=BotAssistant",
        is_bot: true,
  ...
  return (
    <div className="w-[300px] min-w-[300px] border-r h-[85vh] max-h-[750px] my-8 bg-white border shadow-md rounded-l-xl flex flex-col overflow-hidden">
                      ...
                      {user.last_message ? (
                        <>
                          ...
                          {user.last_message.length > 25
                            ? user.last_message.substring(0, 25) + "..."
                            : user.last_message}
                        </>
                      ) : user.is_bot ? (
                        "Assistente AI"
