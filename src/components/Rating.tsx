...
export default function Rating({ 
  ...
  const handleRatingSubmit = async () => {
    ...
    // Check if review contains profanity before submitting
    if (containsProfanity(review)) {
      toast.warning("Sua avaliação contém palavras inapropriadas que foram substituídas.");
    }
    
    // Filter the review for profanity
    const filteredReview = filterProfanity(review);
    ...
    try {
      await addRating(
        ...
        itemType,
        rating,
        filteredReview // Use the filtered review
      );
